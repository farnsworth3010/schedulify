...
def xlsToMysql(filename, course, faculty):
    ...
    def checkDay(start, dayname, day_number, group_id, group_name, firstLetter, secondLetter):
        ...
        for i in range(1, 9):  # Проверяем 8 пар
            ...
            if (teacherFirstGroup or teacherSecondGroup or audienceFirstGroup or audienceSecondGroup):
                try:
                    print(logTime()+f"[{days[int(day_number)-1]} {subjectName} 1:{str(audienceSecondGroup)} 2:{
                          str(audienceSecondGroup)} 1:{teacherFirstGroup} 2:{teacherSecondGroup}]")
                    if (subjectFirstGroup or subjectSecondGroup):
                        cursor.execute("""INSERT INTO `schedule` (id, lesson_number, audience, group_id, day_number, subject, subgroup_id, teacher, group_name, course, faculty, upd) VALUES (NULL, '"""+str(i)+"""', '"""+str(audienceFirstGroup) +
                                       """', '"""+str(group_id)+"""', '"""+str(day_number)+"""', '"""+subjectFirstGroup+"""', '1', '"""+teacherFirstGroup+"""', '"""+group_name+"""', '"""+str(course)+"""', '"""+str(faculty)+"""', '"""+schedDate()+"""');""")
                        cursor.execute("""INSERT INTO `schedule` (id, lesson_number, audience, group_id, day_number, subject, subgroup_id, teacher, group_name, course, faculty, upd) VALUES (NULL, '"""+str(i)+"""', '"""+str(audienceSecondGroup) +
                                       """', '"""+str(group_id)+"""', '"""+str(day_number)+"""', '"""+subjectSecondGroup+"""', '2', '"""+teacherSecondGroup+"""', '"""+group_name+"""', '"""+str(course)+"""', '"""+str(faculty)+"""', '"""+schedDate()+"""');""")
                    else:
                        cursor.execute("""INSERT INTO `schedule` (id, lesson_number, audience, group_id, day_number, subject, subgroup_id, teacher, group_name, course, faculty, upd) VALUES (NULL, '"""+str(i)+"""', '"""+str(audienceFirstGroup) +
                                       """', '"""+str(group_id)+"""', '"""+str(day_number)+"""', '"""+subjectName+"""', '1', '"""+teacherFirstGroup+"""', '"""+group_name+"""', '"""+str(course)+"""', '"""+str(faculty)+"""', '"""+schedDate()+"""');""")
                        cursor.execute("""INSERT INTO `schedule` (id, lesson_number, audience, group_id, day_number, subject, subgroup_id, teacher, group_name, course, faculty, upd) VALUES (NULL, '"""+str(i)+"""', '"""+str(audienceSecondGroup) +
                                       """', '"""+str(group_id)+"""', '"""+str(day_number)+"""', '"""+subjectName+"""', '2', '"""+teacherSecondGroup+"""', '"""+group_name+"""', '"""+str(course)+"""', '"""+str(faculty)+"""', '"""+schedDate()+"""');""")
                    connect.commit()
                ...
                try:
                    cursor.execute("""INSERT INTO `schedule` (id, lesson_number, audience, group_id, day_number, subject, subgroup_id, teacher, group_name, course, faculty, upd) VALUES (NULL, '"""+str(i)+"""', '"""+str(audienceOfBothGroups) +
                                   """', '"""+str(group_id)+"""', '"""+str(day_number)+"""', '"""+subjectName+"""', NULL, '"""+teacherOfBothGroups+"""', '"""+group_name+"""', '"""+str(course)+"""', '"""+str(faculty)+"""', '"""+schedDate()+"""');""")
                    print(logTime()+f"[{days[int(day_number)-1]} {subjectName} {
                          str(audienceOfBothGroups)} {teacherOfBothGroups}]")
                    connect.commit()
                except mysql.connector.Error as err:
                    print(logTime()+"Something went wrong: {}".format(err))
    ...
    for i in range(0, 8):  # Проверяем каждый день каждой группы
        if (getCellVal(sheet, firstLetters[i], startfrom-2)):
            ...
            checkDay(monday, "Понедельник", "1",  int(str(course)+str(i)),
                     groups[i], firstLetters[i], secondLetters[i])
            checkDay(tuesday, "Вторник", "2", int(str(course)+str(i)),
                     groups[i],  firstLetters[i], secondLetters[i])
            checkDay(wednesday, "Среда", "3", int(str(course)+str(i)),
                     groups[i], firstLetters[i], secondLetters[i])
            checkDay(thursday, "Четверг", "4", int(str(course)+str(i)),
                     groups[i],  firstLetters[i], secondLetters[i])
            checkDay(friday, "Пятница", "5", int(str(course)+str(i)),
                     groups[i], firstLetters[i], secondLetters[i])
            checkDay(saturday, "Суббота", "6", int(str(course)+str(i)),
                     groups[i],  firstLetters[i], secondLetters[i])
    ...
    for x in links:
        if x.get("href").find('занятий') != -1:
            try:
                ...
                savedlink = open("savedlink" + str(course)+".txt").read()
                ...
                if (savedlink == linkstart+x.get("href")):
                    print(logTime()+colors.OKGREEN+"Schedule for course " +
                          str(course)+" is up to date :)!"+colors.ENDC)
                    course = course + 1
                    continue
                else:
                    open("savedlink"+str(course)+".txt",
                         'w').write(linkstart+x.get("href"))
                    print(logTime()+colors.WARNING+colors.WARNING +
                          "starting update..."+colors.ENDC)
                    cursor.execute("""DELETE FROM schedule WHERE course = """ +
                                   str(course) + """""")  # Стираем базу данных :)
